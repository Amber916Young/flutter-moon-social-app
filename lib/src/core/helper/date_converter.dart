...
class DateConverter {
  static String formatDate(DateTime? dateTime, BuildContext context, {bool isSecond = true}) {
    return isSecond ? DateFormat('yyyy-MM-dd ${_timeFormatter(context)}:ss').format(dateTime!) : DateFormat('yyyy-MM-dd ${_timeFormatter(context)}').format(dateTime!);
  }
  ...
  static DateTime convertStringToDatetime(String dateTime) {
    return DateFormat("yyyy-MM-dd HH:mm:ss.SSS").parse(dateTime);
  }
  ...
  static String convertDataTimeRange(DateTime startTime, DateTime endTime) {
  ...
  // method to get the date formated like this : 15 October 2023 at 14:30
  static String getFormattedDate(DateTime dateTime) {
    return DateFormat('dd MMMM yyyy').format(dateTime) + ' at ' + DateFormat('HH:mm').format(dateTime);
  }
}
